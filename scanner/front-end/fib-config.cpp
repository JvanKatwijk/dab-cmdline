...
int	fibConfig::getServiceComp	(const std::string &service) {
...
int	fibConfig::get_nrComponents	() {
	return SC_C_table. size ();
}

//	
contentType	fibConfig::content (int compnr) {
	serviceComp_C &comp = SC_C_table [compnr];
	contentType theData;
	theData. TMid	= comp. TMid;
	theData. SId		= comp. SId;
	theData. primary	= comp. compNr == 0;
