...
//
//
	fibDecoder::fibDecoder (API_struct *parent, void *userData) {
	(void)userData;
...
//	   int utc_seconds	= currentConfig -> dateTime [5];
	   ...
	   timeHandler (currentConfig -> dateTime [2],
	                 currentConfig -> dateTime [3], nullptr);
	...
	if (!shown) {
	   if (theEnsemble. namePresent) {
	      fprintf (stderr, "FIG0/20 appears in %s\n",
	                           theEnsemble. ensembleName. c_str ());
	      shown = true;
	   }
	}
#endif
...
//	charSet 0 .. 15, 4 bits, checks are superfluous
	...
	if (!theEnsemble. namePresent) {
	   theEnsemble. ensembleName	= name;
	   theEnsemble. EId		= EId;
	   theEnsemble. namePresent	= true;
	   name_of_ensemble (name, EId, nullptr);
	}
...
//	we do not want it in the list
	for (auto &prim : theEnsemble. primaries)
	   if (prim. name == seco. name)
	      return;
	serviceName  (dataName, SId, -1, nullptr);
}
...
int	fibDecoder::get_nrComponents	() {
int res;
	fibLocker. lock ();
	res	= currentConfig -> get_nrComponents ();
	fibLocker. unlock ();
	return res;
}

contentType fibDecoder::content (int ind) {
contentType res;
	fibLocker. lock ();
	res = currentConfig -> content (ind);
	fibLocker. unlock ();
	return res;
}
...
uint16_t fibDecoder::get_EId	() {
	return theEnsemble. EId;
}

std::string fibDecoder::get_ensembleName	() {
	return theEnsemble. namePresent ? theEnsemble. ensembleName : "";
}
