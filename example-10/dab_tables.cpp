...
// folowing countryTable[] is sorted over all regions to make it easier to find "missing" / "old"
//   but still unused entries. achieved by searching old standard tables document for each ECC code.
...
const char * getProgramType(bool gotInterTabId, uint8_t interTabId, int16_t programType) {
    if ( gotInterTabId && interTabId == 1 )
        return getProgramType_Not_NorthAmerica(programType);
    else if ( gotInterTabId && interTabId == 2 )
