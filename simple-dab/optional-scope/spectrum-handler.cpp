...
bool	isValid (float x) {
	return (x != numeric_limits<float>::infinity ()) && (x == x);
}
	     

	spectrumhandler::spectrumhandler	(simpleDab	*mr,
...
void	spectrumhandler::showSpectrum	(int32_t vfoFrequency) {
...
void	spectrumhandler::showIQ	(void) {
std::complex<float> *Values	=
	 (std::complex<float> *)alloca (1536 * sizeof (std::complex<float>));
int16_t	i;
int16_t	t;
double	avg	= 0;
int	scopeWidth	= scopeSlider -> value ();

	if (iqBuffer -> GetRingBufferReadAvailable () < 256)
	   return;
	t = iqBuffer -> getDataFromBuffer (Values, 256);
