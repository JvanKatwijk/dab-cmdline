...
std::list<section>& config::get_sections (void) {
	return sections;
}
...
//	this is a new section so if we have a current
//	section populated, add it to list
...
//	this is a new section so if we have a current section populated,
//	add it to list
	if (!currentsection.name.empty()) {
	   sections. push_back (currentsection);  // copy
	   currentsection. name = "";
